...
begin
 ...
 if p_date is null or p_first is null or p_first=p_second then raise exception 'Velg minst én person, og ikke samme person to ganger';end if;
 if (select count(*) from public.memberships where stable_id=p_stable and user_id in(p_first,p_second) and active)<>(case when p_second is null then 1 else 2 end) then raise exception 'Velg aktive personer i stallen';end if;
 ...
 values(p_stable,p_date,'23:59:59',case when p_second is null then 'Dagsansvar: morgen, lunsj og kveld' else 'Felles ansvar: morgen, lunsj og kveld' end,p_first,p_first,auth.uid(),1);
 if p_second is not null then
 insert into public.feeding_shifts(stable_id,shift_date,shift_time,label,assigned_to,original_assigned_to,created_by,daily_slot)
 values(p_stable,p_date,'23:59:59','Felles ansvar: morgen, lunsj og kveld',p_second,p_second,auth.uid(),2);
 end if;
end;
$$;
