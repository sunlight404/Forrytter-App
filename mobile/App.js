...
function TodayScreen({ userId }) {
  ...
  return <>
    <Section title="Min oversikt"><Text style={styles.muted}>{formatDateLong(today)} · {weekLabel(today)} · Versjon 1.8.0</Text>{error?<Text accessibilityRole="alert">{error}</Text>:null}<Btn title={busy?'Oppdaterer …':'Oppdater'} disabled={busy} secondary onPress={()=>{load();setTaskRevision(v=>v+1);}}/></Section>
...
function AdminScreen({ currentUserId, role }) {
  ...
  function addShift(){
    if(!feedUser||!feedSecond||feedUser===feedSecond)return Alert.alert('Velg to forskjellige personer');if(feedWorking.current||feedLoading||feedError)return;
    const expected=adminFeeds.filter(s=>s.shift_date===feedDate).map(s=>s.id);
    const first=feedUser,second=feedSecond,date=feedDate;
    async function save(){if(feedWorking.current)return;feedWorking.current=true;setFeedSaving(true);try{const {error}=await supabase.rpc('save_daily_feeding',{p_stable:STABLE_ID,p_date:date,p_first:first,p_second:second,p_expected:expected});if(error)throw error;await Promise.all([loadAdminFeeds(),load()]);Alert.alert('Lagret','De to personene har felles ansvar for morgen, lunsj og kveld '+formatDate(date)+'.');}catch(e){Alert.alert('Kunne ikke lagre dagsansvar',e.message);}finally{feedWorking.current=false;setFeedSaving(false);}}
    if(expected.length)Alert.alert('Erstatt fôringer denne dagen',formatDateLong(date)+': Eksisterende registreringer erstattes med felles dagsansvar for '+nameOf(first)+' og '+nameOf(second)+'.',[{text:'Avbryt',style:'cancel'},{text:'Lagre dagsansvar',onPress:save}]);else save();
  }

  const sections=[['profiles','Brukerprofiler','Finn en bruker, godkjenn tilgang og endre rolle.'],['agreements','Faste avtaler','Rediger faste helger, hest og trening.'],['training','Hest og trening','Endre hest eller trening på én bestemt dato.'],['tasks','Oppgaver','Legg til og fjern oppgaver for en hestedag.'],['feeding','Fôringer','Velg to personer med felles ansvar for hele dagen.'],['horses','Hester','Legg til eller fjern hester fra listen.'],['messages','Beskjeder','Publiser en beskjed til alle.']];
