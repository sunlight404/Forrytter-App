...
function TodayScreen({ userId }) {
  ...
  return <>
    <Section title="Min oversikt"><Text style={styles.muted}>{formatDateLong(today)} · {weekLabel(today)} · Versjon 1.8.1</Text>{error?<Text accessibilityRole="alert">{error}</Text>:null}<Btn title={busy?'Oppdaterer …':'Oppdater'} disabled={busy} secondary onPress={()=>{load();setTaskRevision(v=>v+1);}}/></Section>
...
function AdminScreen({ currentUserId, role }) {
  ...
  const [feedSecond,setFeedSecond]=useState(null);
  ...
  function addShift(){
    if(!feedUser)return Alert.alert('Velg minst én person');if(feedSecond&&feedUser===feedSecond)return Alert.alert('Velg to forskjellige personer');if(feedWorking.current||feedLoading||feedError)return;
    const expected=adminFeeds.filter(s=>s.shift_date===feedDate).map(s=>s.id);
    const first=feedUser,second=feedSecond,date=feedDate;
    async function save(){if(feedWorking.current)return;feedWorking.current=true;setFeedSaving(true);try{const {error}=await supabase.rpc('save_daily_feeding',{p_stable:STABLE_ID,p_date:date,p_first:first,p_second:second,p_expected:expected});if(error)throw error;await Promise.all([loadAdminFeeds(),load()]);Alert.alert('Lagret','Dagsansvar for morgen, lunsj og kveld er lagret '+formatDate(date)+'.');}catch(e){Alert.alert('Kunne ikke lagre dagsansvar',e.message);}finally{feedWorking.current=false;setFeedSaving(false);}}
    if(expected.length)Alert.alert('Erstatt fôringer denne dagen',formatDateLong(date)+': Eksisterende registreringer erstattes med felles dagsansvar for '+nameOf(first)+(second?' og '+nameOf(second):'')+'.',[{text:'Avbryt',style:'cancel'},{text:'Lagre dagsansvar',onPress:save}]);else save();
  }

  const sections=[['profiles','Brukerprofiler','Finn en bruker, godkjenn tilgang og endre rolle.'],['agreements','Faste avtaler','Rediger faste helger, hest og trening.'],['training','Hest og trening','Endre hest eller trening på én bestemt dato.'],['tasks','Oppgaver','Legg til og fjern oppgaver for en hestedag.'],['feeding','Fôringer','Velg én eller to personer med ansvar for hele dagen.'],['horses','Hester','Legg til eller fjern hester fra listen.'],['messages','Beskjeder','Publiser en beskjed til alle.']];
  ...
  return <>
    ...
    <View style={{display:adminPanel==='feeding'?'flex':'none'}}><Section title="Fôring – én eller to personer per dag"><Text style={styles.help}>Én person kan ha ansvaret alene. Velger du to, har de ansvaret sammen for morgen, lunsj og kveld. En prikk i kalenderen betyr at dagen har registreringer.</Text><Btn title={feedLoading?'Oppdaterer …':'Oppdater fôringer'} secondary disabled={feedLoading||feedSaving} onPress={loadAdminFeeds}/><CalendarPicker value={feedDate} onChange={date=>{setFeedDate(date);setFeedMonth(date);setFeedUser(null);setFeedSecond(null);}} onMonthChange={setFeedMonth} markedDates={[...new Set(adminFeeds.map(s=>s.shift_date))]}/>{!!feedError&&<Text accessibilityRole="alert">{feedError}</Text>}<Text style={styles.bold}>{formatDateLong(feedDate)}</Text>{[1,2].map(slot=>{const shift=adminFeeds.find(s=>s.shift_date===feedDate&&s.daily_slot===slot);return <View key={slot} style={styles.item}><Text style={styles.bold}>Person {slot}</Text><Text>{shift?.assigned_to?(allMembers.find(m=>m.user_id===shift.assigned_to)?.profiles?.full_name||'Tidligere bruker'):feedLoading?'Henter …':feedError?'Ukjent status':slot===2?'Ingen ekstra person valgt':'Ikke fordelt'}</Text>{shift&&<Btn title="Fjern registrering" danger disabled={feedSaving||feedLoading} onPress={()=>confirmRemoveFeeding(shift)}/>}</View>;})}{adminFeeds.filter(s=>s.shift_date===feedDate&&!s.daily_slot).map(shift=><View key={shift.id} style={styles.item}><Text style={styles.bold}>Tidligere registrering: {feedingLabel(shift)}</Text><Text>{allMembers.find(m=>m.user_id===shift.assigned_to)?.profiles?.full_name||'Ikke fordelt'}</Text><Text style={styles.help}>Gjelder det opprinnelige måltidet til du erstatter dagen med felles ansvar.</Text><Btn title="Fjern registrering" danger disabled={feedSaving||feedLoading} onPress={()=>confirmRemoveFeeding(shift)}/></View>)}<Text style={styles.sectionSmall}>Registrer eller endre ansvarlige</Text><Dropdown label="Person 1" value={feedUser} onChange={setFeedUser} options={memberOptions(members.filter(m=>m.user_id!==feedSecond))}/><Dropdown label="Person 2 (valgfri)" value={feedSecond} onChange={setFeedSecond} options={memberOptions(members.filter(m=>m.user_id!==feedUser))}/><Btn title="Ingen person 2" secondary disabled={feedSaving} onPress={()=>setFeedSecond(null)}/><Text style={styles.help}>Morgen, lunsj og kveld – alene eller sammen med person 2.</Text><Btn title={feedSaving?'Lagrer …':'Lagre dagsansvar'} disabled={feedSaving||feedLoading||!!feedError||!feedUser||feedUser===feedSecond} onPress={addShift}/></Section></View>
  </>;
}
